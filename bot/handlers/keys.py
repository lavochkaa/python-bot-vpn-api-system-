...
def _telegram_safe_open_url(deep_link: str) -> str:
    # Telegram inline keyboard URL accepts mostly http/https/tg schemes.
    # For custom app schemes we provide a safe Telegram share-url wrapper.
...
async def connect_apps(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    if not keys:
        ...
        await edit_or_send(
            ...
            "TODO(VPNPANEL): выдать конфиг сразу после активации подписки.",
            reply_markup=builder.as_markup(),
        )
        await call.answer()
        return
    ...
    vpn_key = keys[0]
    normalized_key = _normalize_subscription_link(vpn_key.key, call.from_user.id)
    # If provider already returned a ready subscription URL, open it directly.
    if normalized_key.startswith(("https://", "http://")):
        safe_url = normalized_key.replace("<", "").replace(">", "")
        try:
            await call.answer(url=normalized_key)
        except Exception:
            await call.answer()
    ...
    await send_or_answer(
        ...
        "Нажмите «Как подключиться?», чтобы открыть инструкцию с приложениями "
        "(Hiddify / v2 / Streisand / Happ).",
        reply_markup=_direct_subscription_keyboard(),
    )
    await call.answer()
...
async def connect_pick_app(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    keys = await VpnKeyRepository(session).get_user_keys(call.from_user.id, limit=1)
    if not keys:
        await call.answer("Конфиг еще не готов.", show_alert=True)
        return
    vpn_key = keys[0]
    deep_link = _build_deep_link(app, _normalize_subscription_link(vpn_key.key, call.from_user.id))
    open_url = _telegram_safe_open_url(deep_link)

    # Try instant redirect on app choice click.
    try:
